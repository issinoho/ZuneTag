...
- Versioning: before building, the workflow computes the next release version from existing `vX.Y.Z` git tags (bumping the patch number) and rewrites `Properties/AssemblyInfo.cs`'s `AssemblyVersion`/`AssemblyFileVersion` attributes to match, so the assembly version baked into a given build (visible in the About box) always matches the GitHub release tag created from that build's artifact. The computed version is printed in the "Determine next version" step's log — read it back from there when tagging/releasing rather than guessing the next number. `Properties/AssemblyInfo.cs`'s checked-in version is just a placeholder for local/dev builds.
